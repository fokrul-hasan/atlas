...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await getPostBySlug("recipes", slug);
  if (!post) return {};
...
export default async function RecipePostPage({ params }: Props) {
  const { slug: rawSlug } = await params;
  const slug = safeDecode(rawSlug);
  const post = await getPostBySlug("recipes", slug);
  if (!post) notFound();
      ...
      <article>
        ...
        {(prev || next) && (
          <nav className="prev-next">
            {prev ? (
              <Link href={`/recipes/${prev.slug}`} className="prev-item">
                <span className="dir">← Previous</span>
                <span className={`ptitle${prev.language === "bn" ? " bn" : ""}`}>{prev.title}</span>
              </Link>
            ) : <span />}
            {next ? (
              <Link href={`/recipes/${next.slug}`} className="next-item">
