...
export default async function PlaygroundPostPage({ params }: Props) {
  ...
  return (
    <main className="wrap">
      ...
      <Link href="/playground" className="back-link">← Back to Playground</Link>
      ...
      <article>
        ...
        {(prev || next) && (
          <nav className="prev-next">
            {prev ? (
              <Link href={`/playground/${prev.slug}`} className="prev-item">
                <span className="dir">← Previous</span>
                <span className={`ptitle${prev.language === "bn" ? " bn" : ""}`}>{prev.title}</span>
              </Link>
            ) : <span />}
            {next ? (
              <Link href={`/playground/${next.slug}`} className="next-item">
